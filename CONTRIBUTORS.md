...
- `release-*` — maintenance branches used to continue producing Fedora older release images and receive Renovate updates. Builds from this branch publish the tags with each major release version.
...
------------------------------
Renovate is configured to create automated dependency update PRs against `main` and `release-*` so maintenance branches continue to receive security and patch updates.
